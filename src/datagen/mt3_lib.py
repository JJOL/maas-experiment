...
def get_audio(wav_file, sample_rate):
  with open(wav_file, 'rb') as wav_file:
    return note_seq.audio_io.wav_data_to_samples_librosa(wav_file.read(), sample_rate=sample_rate)
...
def transcribe(wav, base_path, output):
  ...
  note_seq.sequence_proto_to_midi_file(est_ns, f"{output}.mid")